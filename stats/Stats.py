...
class Stats:
    ...
    def saveAllContainerInfo(self):
        containerinfo = dict()
        allCreatedContainers = [self.env.getContainerByCID(cid) for cid in
    ...
    def runSimpleSimulation(self, decision):
        ...
        return energytotalinterval_pred * self.env.intervaltime, max(0, np.mean(
        ...
        for hostID in range(len(listinfo[0][metric])):
            ...
            if metric2:
                metric2_with_interval.append(
                    [listinfo[interval][metric2][hostID] for interval in range(totalIntervals)])
    ...
    def generateMetricsWithInterval(self, dirname):
        ...
        for i, metric in enumerate(['numdestroyed', 'nummigrations', 'energytotalinterval', 'avgresponsetime', \
            ...
            metric_with_interval = [self.metrics[i][metric] for i in
                                    range(len(self.metrics))] if metric != 'waittime' else \
        ...
        print('Average energy (sum energy interval / sum numdestroyed) = ',
    ...
    def generateDatasets(self, dirname):
        ...
        self.generateDatasetWithInterval(dirname, 'cpu', metric2='apparentips', objfunc='energytotalinterval',
                                         objfunc2='avgresponsetime')
        self.generateDatasetWithInterval2(dirname, 'cpu', 'apparentips', 'energytotalinterval_pred',
                                          'avgresponsetime_pred', objfunc='energytotalinterval',
                                          objfunc2='avgresponsetime')
