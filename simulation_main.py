...
def step_simulation(workload, scheduler, env, stats):
    ...
    stats.save_stats(deployed, migrations, [], selected, decision, scheduling_time, fitness=fitness)
    destroyed = env.destroy_completed_containers()
    stats.save_metrics(destroyed, migrations)
...
def save_stats(dirname, stats, env, end=True):
...
def static_main():
    ...
    for step in range(sim_steps):
        ...
        if env != '' and step % 10 == 0:
            save_stats(dirname, stats, env, end=False)

    save_stats(dirname, stats, env)
