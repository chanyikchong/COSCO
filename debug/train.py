...
def backprop(dataset, model, optimizer):
    ...
    for feat in dataset:
        ...
        # print(y_pred, y_true)
        optimizer.zero_grad()
        loss = torch.sum((y_pred - y_true) ** 2) if 'stochastic' not in model.name else KL_loss(y_pred,
                                                                                                Coeff_Energy * y_true[
                                                                                                    0] + Coeff_Latency *
                                                                                                y_true[1])
        loss.backward()
...
if __name__ == '__main__':
    data_type = argv[
        1]  # can be 'energy', 'energy_latency', 'energy_latency2', 'stochastic_energy_latency', 'stochastic_energy_latency2' + '_' + str(HOSTS)
    ...
    if exec_type == "train":
        ...
        print("The minimum loss on test set is ", str(min(accuracy_list)), " at epoch ",
