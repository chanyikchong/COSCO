...
class RequestRouter():
        ...
        for container in containers:
            ...
            if running:
                ...
                if stats["memory_stats"]["limit"] != 0:
                    memory_percent = float(stats["memory_stats"]["usage"]) / float(
                        stats["memory_stats"]["limit"]) * 100.0
                try:
                    disk_stat = subprocess.run(["docker", "ps", "--size", "-a"], universal_newlines=True,
    ...
    def migrate(self, payload):
        ...
        try:
            cid = subprocess.run("docker inspect -f '{{.Id}}' " + container_name, shell=True, stdout=subprocess.PIPE)
            cid = cid.stdout.decode('utf-8').strip()
            running = checkpoint_name in subprocess.run(["docker", 'checkpoint', 'ls', container_name],
    ...
    def restore(self, payload):
        ...
        try:
            ...
            if running:
                cmd = "sudo tar -zxf /tmp/" + container_name + "." + checkpoint_name + ".tgz -C /var/lib/docker/containers/" + str(
                    cid) + "/" + "checkpoints/"
                subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, universal_newlines=True)
                subprocess.call(["sudo", "rm", "-rf", "/tmp/" + container_name + "." + checkpoint_name + ".tgz"])
        except ValueError:
        ...
        if running:
            output = subprocess.run(["docker", "start", "--checkpoint", checkpoint_name, container_name],
                                    stderr=subprocess.PIPE)
            output = output.stderr.decode()
            subprocess.call(["sudo", "docker", "checkpoint", "rm", container_name, checkpoint_name])
        else:
            output = 'INFO : Container ' + container_name + " was not running in source host so has not been restored"
        if 'Error' in output:
