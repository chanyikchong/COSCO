...
touch output.txt
cpus=$(nproc)
sudo perf stat -o output.txt -e instructions -e task-clock sysbench --num-threads=$cpus --test=cpu run &>/dev/null || sudo myperf stat -o output.txt -e instructions -e task-clock sysbench --num-threads=$cpus --test=cpu run &>/dev/null
