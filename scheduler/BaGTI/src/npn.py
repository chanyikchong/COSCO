...
class NPNRelu(nn.Module):
    ...
    def forward(self, x):
        assert (len(x) == 2)
        o_m, o_s = x
        a_m = F.sigmoid(self.scale * o_m * (o_s ** (-0.5))) * o_m + torch.sqrt(o_s) / math.sqrt(
            2 * math.pi) * torch.exp(-o_m ** 2 / (2 * o_s))
        a_s = F.sigmoid(self.scale * o_m * (o_s ** (-0.5))) * (o_m ** 2 + o_s) + o_m * torch.sqrt(o_s) / math.sqrt(
            2 * math.pi) * torch.exp(-o_m ** 2 / (2 * o_s)) - a_m ** 2  # mbr
        return a_m, a_s
...
def NPNBCELoss(pred_m, pred_s, label):
    loss = -torch.sum((torch.log(pred_m + 1e-10) * label + torch.log(1 - pred_m + 1e-10) * (1 - label)) / (
                pred_s + 1e-10) - torch.log(pred_s + 1e-10))
    return loss


def KL_BG(pred_m, pred_s, label):
    loss = 0.5 * torch.sum(
        (1 - label) * (pred_m ** 2 / pred_s + torch.log(torch.clamp(math.pi * 2 * pred_s, min=1e-6))) + label * (
                    (pred_m - 1) ** 2 / pred_s + torch.log(torch.clamp(math.pi * 2 * pred_s, min=1e-6)))) / \
           pred_m.size()[0]  # min = 1e-6
    return loss
