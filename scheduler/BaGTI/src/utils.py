...
def load_energy_latency_data(hosts):
    ...
    print("Dataset size", data.shape[0])
    for i in range(data.shape[0]):
        cpu_host, cpu_container, alloc = [], [], []
        for j in range(hosts):
            cpu_host.append(data[i][j] / 100)
            cpu_container.append(data[i][j + hosts] / max_ips_container)
            one_hot = [0] * hosts
            if int(data[i][j + (2 * hosts)]) >= 0:
                one_hot[int(data[i][j + (2 * hosts)])] = 1
            alloc.append(one_hot)
        cpu_host = np.array([cpu_host]).transpose()
        cpu_container = np.array([cpu_container]).transpose()
        alloc = np.array(alloc)
        dataset.append(((np.concatenate((cpu_host, cpu_container, alloc), axis=1)), torch.Tensor(
            [(data[i][-2] - data.min(0)[-2]) / (data.max(0)[-2] - data.min(0)[-2]),
             max(0, data[i][-1]) / data.max(0)[-1]])))
        if dataset[-1][1][1] > 1:
            print(dataset[-1])
    # Normalization by (x - min)/(max - min)
...
def load_energy_latency2_data(hosts):
    ...
    data = pd.read_csv(dataset_path, header=None) if os.path.exists(dataset_path) else pd.read_csv(
        'scheduler/BaGTI/' + dataset_path, header=None)
    data = data.values.astype(np.float)
    max_ips_container = max(data.max(0)[hosts:2 * hosts])
    ...
    for i in range(data.shape[0]):
        ...
        cpu_container = np.array([cpu_container]).transpose()
        alloc = np.array(alloc)
        pred_vals = np.broadcast_to(np.array([data[i][3 * hosts] / max_energy, data[i][3 * hosts + 1] / max_response]),
                                    (hosts, 2))
        dataset.append(((np.concatenate((cpu_host, cpu_container, alloc, pred_vals), axis=1)), torch.Tensor(
            [(data[i][-2] - data.min(0)[-2]) / (data.max(0)[-2] - data.min(0)[-2]),
             max(0, data[i][-1]) / data.max(0)[-1]])))
    # Normalization by (x - min)/(max - min)
